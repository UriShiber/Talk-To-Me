...
def convert_m4a_to_wav(file_name):
    ...
    audio_file = os.path.join(RECORDING_DIR, "record_try_{0}.wav".format(counter))
    audio_signal_segment = audio_signal_segment.set_channels(wave_file.getnchannels())
    audio_signal_segment = audio_signal_segment.set_frame_rate(wave_file.getframerate())
    audio_signal_segment = audio_signal_segment.set_sample_width(wave_file.getsampwidth())
    audio_signal_segment.export(audio_file, format='wav')
    return audio_file


audio_file2 = convert_m4a_to_wav(r"C:\final_proj\my_record1")
